...
router.post('/report/:id', writeLimiter, auth.authMiddleware, validate(createReportSchema), jobReportController.createReport)

// GET /api/job/list - Admin only
// Rate limited: Expensive query with joins
router.get('/list', strictLimiter, auth.authMiddleware, role.roleMiddleware(['ADMIN']), jobReportController.listReports)

// DELETE /api/job/report/:reportId - Admin only
// Rate limited: Write operation
router.delete('/report/:reportId', writeLimiter, auth.authMiddleware, role.roleMiddleware(['ADMIN']), jobReportController.deleteReport)

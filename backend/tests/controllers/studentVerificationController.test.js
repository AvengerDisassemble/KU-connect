...
jest.mock("../../src/services/storageFactory", () => ({
  ...
  getFileUrl: jest
    .fn()
    .mockResolvedValue("https://mock-url.com/student-verification"),
  deleteFile: jest.fn().mockResolvedValue(undefined),
  getReadStream: jest.fn().mockResolvedValue({
    stream: require("stream").Readable.from(
      Buffer.from("%PDF-1.4 mock verification doc"),
    ),
...
describe("Student Verification Controller", () => {
    ...
    try {
      ...
      await prisma.student.deleteMany({
        where: {
          user: {
            email: {
              in: ["unverified-student@test.com", "verified-student@test.com"],
            },
          },
        },
      });
  ...
  describe("GET /api/documents/student-verification/:userId/download - Download verification document", () => {
    ...
    it("should allow admin to download student verification document", async () => {
      const response = await request(app)
        .get(
          `/api/documents/student-verification/${unverifiedStudentUserId}/download`,
        )
        .set("Authorization", `Bearer ${adminToken}`);

...
function downloadRateLimit(req, res, next) {
  ...
  if (userData.count > MAX_DOWNLOADS_PER_WINDOW) {
    return res.status(429).json({
      ...
      retryAfter: Math.ceil(
        (RATE_LIMIT_WINDOW_MS - (now - userData.windowStart)) / 1000,
      ),
    });
  }
  ...
  res.setHeader(
    ...
    Math.max(0, MAX_DOWNLOADS_PER_WINDOW - userData.count),
  );
  res.setHeader(
    "X-RateLimit-Reset",
    new Date(userData.windowStart + RATE_LIMIT_WINDOW_MS).toISOString(),
  );

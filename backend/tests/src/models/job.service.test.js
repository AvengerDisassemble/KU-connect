...
describe("jobService (unit)", () => {
  ...
  describe("createJob()", () => {
    ...
    it("throws 404 if HR not found", async () => {
      ...
      await expect(
        jobService.createJob("nonexistent", {}),
  ...
  describe("updateJob()", () => {
    ...
    it("throws 403 when HR not owner", async () => {
      ...
      await expect(
        jobService.updateJob("job2", "hr99", {}),
  ...
  describe("deleteJob()", () => {
    ...
    it("allows HR owner to delete their own job", async () => {
      ...
      const result = await jobService.deleteJob("job4", {
        role: "EMPLOYER",
        hr: { id: "hr9" },
      });
    ...
    it("throws 403 for non-admin non-employer roles", async () => {
      ...
      await expect(
        jobService.deleteJob("job6", { role: "STUDENT" }),
      ).rejects.toMatchObject({ status: 403 });
    });
    ...
    it("throws 404 if job not found", async () => {
      prisma.job.findUnique.mockResolvedValue(null);
      await expect(
        jobService.deleteJob("nonexistent", { role: "ADMIN" }),
      ).rejects.toMatchObject({ status: 404 });
    });
  });
  ...
  describe("applyToJob()", () => {
    ...
    it("throws 409 if already applied", async () => {
      ...
      await expect(
        jobService.applyToJob("job5", "student7", "resume.pdf"),
  ...
  describe("manageApplication()", () => {
    ...
    it("throws 403 when HR not owner", async () => {
      ...
      await expect(
        jobService.manageApplication("job6", "hr1", "app1", "QUALIFIED"),
      ).rejects.toMatchObject({ status: 403 });
    });
  });
  ...
  describe("getApplicants()", () => {
    ...
    it("throws 403 if HR not owner", async () => {
      ...
      await expect(
        jobService.getApplicants("job7", "hr3"),
      ).rejects.toMatchObject({ status: 403 });
    });
  });

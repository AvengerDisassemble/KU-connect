...
try {
  ...
  console.warn(
    "Skipping storage interface tests - dependencies not installed. Run: npm install uuid mime-types @aws-sdk/client-s3 @aws-sdk/s3-request-presigner",
  );
  depsAvailable = false;
}
...
describeIfDepsAvailable("Storage Provider Interface", () => {
  describe("Base StorageProvider", () => {
    ...
    test("should throw error on uploadFile", async () => {
      ...
      ).rejects.toThrow("uploadFile must be implemented by subclass");
    });

    test("should throw error on getFileUrl", async () => {
      await expect(provider.getFileUrl("test-key")).rejects.toThrow(
        "getFileUrl must be implemented by subclass",
      );
    });

    test("should throw error on deleteFile", async () => {
      await expect(provider.deleteFile("test-key")).rejects.toThrow(

...
function extractUserIdFromReq (req) {
  ...
  const candidates = [req.params && req.params.user_id, req.params && req.params.userId, req.params && req.params.id]
...
async function postSaved (req, res) {
  try {
    ...
    if (err.code === 'NOT_FOUND') {
      return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Job or user not found' } })
    }
    console.error(err)
    console.error(err && err.stack)
    return res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: err.message } })

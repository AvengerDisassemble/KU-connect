...
describe('Student Verification Controller', () => {
  ...
  describe('GET /api/documents/student-verification/:userId - Get verification URL', () => {
    beforeEach(async () => {
    ...
    it('should allow admin to get student verification URL', async () => {
      const response = await request(app)
        .get(`/api/documents/student-verification/${unverifiedStudentUserId}`)
        .set('Authorization', `Bearer ${adminToken}`)

      expect(response.status).toBe(200)
      expect(response.body.success).toBe(true)
      expect(response.body.url).toBeDefined()
      expect(storageProvider.getFileUrl).toHaveBeenCalled()
    })

    it('should allow student to get their own verification URL', async () => {
      const response = await request(app)
        .get(`/api/documents/student-verification/${unverifiedStudentUserId}`)
        .set('Authorization', `Bearer ${unverifiedStudentToken}`)
    ...
    it('should deny other students from accessing verification URL', async () => {
      const response = await request(app)
        .get(`/api/documents/student-verification/${unverifiedStudentUserId}`)
        .set('Authorization', `Bearer ${verifiedStudentToken}`)

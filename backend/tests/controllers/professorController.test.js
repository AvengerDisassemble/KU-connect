...
describe("Professor Analytics Controller", () => {
        ...
        const job = await prisma.job.create({
          data: {
            ...
            application_deadline: new Date(
              now.getTime() + (i % 2 === 0 ? 30 : -30) * 24 * 60 * 60 * 1000,
            ),
        ...
        for (let j = 0; j < numApplications; j++) {
          ...
          const application = await prisma.application.create({
            data: {
              ...
              createdAt: new Date(
                now.getTime() -
                  daysAgo[(i + j) % daysAgo.length] * 24 * 60 * 60 * 1000,
              ),
            },
          });
  ...
  describe("GET /api/professor/analytics/dashboard", () => {
    test("should return dashboard analytics for professor", async () => {
      ...
      expect(
        response.body.data.applicationMetrics.byStatus.pending,
      ...
      expect(response.body.data.applicationTrends.monthly).toBeInstanceOf(
        Array,
      );
    ...
    test("should filter by degreeTypeId", async () => {
      ...
      if (breakdown.length > 0) {
        expect(
          breakdown.every((dt) => dt.degreeTypeId === degreeType1.id),
        ).toBe(true);
      }
    });
    ...
    test("should return 401 for unauthenticated requests", async () => {
      await request(app).get("/api/professor/analytics/dashboard").expect(401);
    });
  });
    ...
    test("should sort by applications (desc)", async () => {
      ...
      if (students.length > 1) {
        ...
        for (let i = 0; i < students.length - 1; i++) {
          expect(students[i].applicationStats.total).toBeGreaterThanOrEqual(
            students[i + 1].applicationStats.total,
          );
        }
      }
    });
  ...
  describe("GET /api/professor/students/:studentId", () => {
    test("should return student detail with full data", async () => {
      ...
      expect(
        response.body.data.student.applicationStatistics.total,
      ).toBeDefined();
      expect(
        response.body.data.student.applicationStatistics.byStatus,
      ).toBeDefined();
    ...
    test("should not include sensitive data", async () => {
      ...
      expect(typeof response.body.data.student.academicInfo.hasResume).toBe(
        "boolean",
      );

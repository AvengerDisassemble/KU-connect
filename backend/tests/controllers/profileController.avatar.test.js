...
jest.mock("../../src/services/storageFactory", () => ({
  uploadFile: jest
    .fn()
    .mockResolvedValue("avatars/user-123/avatar-timestamp.jpg"),
...
describe("Profile Controller - Avatar Routes", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      professorToken = jwt.sign(
        { id: professorUserId, role: "PROFESSOR" },
        secret,
        { expiresIn: "1h" },
      );
    } catch (error) {
  ...
  describe("POST /api/profile/avatar", () => {
    test("should upload avatar for authenticated student", async () => {
      ...
      expect(response.body.data.fileKey).toBe(
        "avatars/user-123/avatar-timestamp.jpg",
      );
    ...
    test("should delete old avatar when uploading new one", async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(storageFactory.deleteFile).toHaveBeenCalledWith(
    ...
    test("should continue if old avatar deletion fails", async () => {
      ...
      storageFactory.deleteFile.mockRejectedValueOnce(
    ...
    test("should update user avatarKey in database", async () => {
      ...
      expect(updatedUser.avatarKey).toBe(
        "avatars/user-123/avatar-timestamp.jpg",
      );
    });
    ...
    test("should handle upload service errors gracefully", async () => {
      // Mock upload to fail
      storageFactory.uploadFile.mockRejectedValueOnce(
  ...
  describe("GET /api/profile/avatar/:userId/download", () => {
    ...
    test("should allow user to download their own avatar", async () => {
      ...
      expect(storageFactory.getReadStream).toHaveBeenCalledWith(
        "avatars/student-avatar.jpg",
      );
    });
    ...
    test("should reject unauthenticated requests", async () => {
      const response = await request(app).get(
        `/api/profile/avatar/${studentUserId}/download`,
      );
    ...
    test("should handle signed URL redirect for S3 storage", async () => {
      // Mock S3 signed URL
      storageFactory.getSignedDownloadUrl.mockResolvedValueOnce(
      ...
      expect(storageFactory.getSignedDownloadUrl).toHaveBeenCalledWith(
        "avatars/student-avatar.jpg",
      );
    });
    ...
    test("should return 404 if file not found in storage", async () => {
      ...
      storageFactory.getReadStream.mockRejectedValueOnce(

...
describeIfS3('S3StorageProvider (integration tests)', () => {
  ...
  describe('constructor', () => {
    ...
    test('should throw error when env vars are missing', () => {
      ...
      expect(() => new S3StorageProvider()).toThrow('S3 provider missing required environment variables')
...
if (!hasS3Config) {
  describe('S3StorageProvider (skipped)', () => {
    test('S3 tests require AWS credentials in environment', () => {
      console.warn('Skipping S3 tests - set AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_REGION, AWS_BUCKET_NAME to run')

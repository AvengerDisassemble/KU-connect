...
async function testNewRegistrationEndpoints() {
  ...
  try {
    const staffResult = await makeRequest('POST', '/api/register/staff', staffData)
  ...
  try {
    const adminResult = await makeRequest('POST', '/api/register/admin', adminData)
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            reject(new Error(`${result.message || 'Unknown error'} (Status: ${res.statusCode})`))
          }
        } catch (error) {
          reject(new Error(`Invalid JSON response: ${body} (Status: ${res.statusCode})`))
        }
      })
    })

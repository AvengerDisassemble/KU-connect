...
export default function EmployerProfilePage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowed.includes(file.type)) return toast.error("Please upload a PDF, JPG, or PNG file.");
    if (file.size > 10 * 1024 * 1024) return toast.error("File size must be less than 10MB.");
    setUploadedFile(file);
  };
  const handleRemoveFile = () => { setUploadedFile(null); toast.info("File removed"); };
  const handleSubmitVerification = async () => {
    if (!uploadedFile) return toast.error("Please upload a document before submitting.");
    ...
    <>
      ...
      <main className="min-h-screen pl-[280px] p-6">
        <div className="max-w-6xl mx-auto">
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-foreground mb-6">Company Profile & Verification</h1>
